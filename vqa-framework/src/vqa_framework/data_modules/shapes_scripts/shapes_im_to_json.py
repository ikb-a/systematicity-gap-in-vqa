                ...
                for y in range(3):
                    ...
                    if np.amax(tmp_flat_grid) == 0:
                        ...
                        if np.amax(flat_grid - np.flipud(flat_grid)) > 0.5:
                            ...
                            """
                            if np.all((flat_grid > 0.6) == (flat_grid > 0)):
                                logging.info("SQUARE")
                            else:
                                logging.info("CIRCLE")
                            """
                            """
                            bool_flat_grid = flat_grid == np.amax(flat_grid) 
                            if np_check_in(bool_flat_grid, SQUARE):
                                logging.info("SQUARE")
                            elif np_check_in(bool_flat_grid, CIRCLE):
                                logging.info("CIRCLE")
                            """
                            corner = None
                            for cy in range(10):
