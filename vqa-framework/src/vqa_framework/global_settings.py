...
""" 
NOTE: Need to setup boto with S3 bucket credentials if using MLFlow remote tracking that stores
artifacts to an S3 bucket. https://boto3.amazonaws.com/v1/documentation/api/latest/guide/configuration.html#guide-configuration
"""
# NOTE: Just use absolute paths for everything.
...
class ComputeEnv(Enum):
    ...
    VECTOR_VAUGHAN_HOME_CKPT = 6
    TORONTO_CS_CLUSTER_SLURM = (
        7  # Toronto DCS, but with different checkpoint saving location
    )
    VECTOR_MARS = 8
...
if COMPUTE_ENV == ComputeEnv.GAME_MLFLOW_LOCAL:
    ...
    comp_env_vars = ConfigVals(
        ...
        TMP_DIR="/media/administrator/3227344f-bcd2-48f5-946e-78d2e7db3d26/vqa-frame",
        LIGHTNING_CHKPT_DIR="/media/administrator/extdrive/vqa-frame_light_chckpts",
        # NOTE: Will need to be updated if the VM is rebooted.
        #       Might be able to setup alias to make dynamic IP not a problem; only bother if wider adoption.
        MLFLOW_URI=IAN_MLFLOW_URI,
        COMPUTE_ENV=COMPUTE_ENV,
    )

    # Ugly hackery. Just Don't Ask. Also blame PyCharm.
    ...
    # Hit-and-miss w.r.t. locking, so screw it. HDF5 file shouldn't be altered anyways.
    ...
    # Hit-and-miss w.r.t. locking, so screw it. HDF5 file shouldn't be altered anyways.
    os.environ["HDF5_USE_FILE_LOCKING"] = "FALSE"
elif COMPUTE_ENV == ComputeEnv.TORONTO_CS_CLUSTER_SLURM:
    ckpt_dir = f"/u/ianberlot/checkpoints/" + os.getenv("SLURM_JOB_ID") + "/"
    ...
    # Hit-and-miss w.r.t. locking, so screw it. HDF5 file shouldn't be altered anyways.
    os.environ["HDF5_USE_FILE_LOCKING"] = "FALSE"
elif COMPUTE_ENV == ComputeEnv.VECTOR_VAUGHAN_HOME_CKPT:
    if "SLURM_JOB_ID" in os.environ:
        # THIS PATH IS BAD! Ideally checkpoints should go to /checkpoint/ianberlot/
        # But if there's no space there, there's no choice.
        ckpt_dir = f'/h/ianberlot/ckpts/{os.environ["SLURM_JOB_ID"]}'
    else:
        logging.warning(
            "ENVIRONMENT: No SLURM job # found; using working dir to store checkpoints"
        )
    ...
    # Hit-and-miss w.r.t. locking, so screw it. HDF5 file shouldn't be altered anyways.
    os.environ["HDF5_USE_FILE_LOCKING"] = "FALSE"
elif COMPUTE_ENV == ComputeEnv.VECTOR_MARS:
    if "SLURM_JOB_ID" in os.environ:
        # 300GB total in gobi1; SSD though, so quicker than HDD in gobi2
        ckpt_dir = f'/scratch/gobi1/ianberlot/ScratchFiles/thunder_ckpts/{os.environ["SLURM_JOB_ID"]}'
    else:
        logging.warning(
            "ENVIRONMENT: No SLURM job # found; using working dir to store checkpoints"
        )
    ...
    comp_env_vars = ConfigVals(
        DATA_DIR="/scratch/gobi2/ianberlot/thunder_datasets",  # NOTE: Not presently in use! Some old data in /scratch/gobi2/ianberlot/datasets
        TMP_DIR="/scratch/gobi2/ianberlot/thunder_tmp",
        LIGHTNING_CHKPT_DIR=ckpt_dir,
        MLFLOW_URI=IAN_MLFLOW_URI,
        COMPUTE_ENV=COMPUTE_ENV,
    )
