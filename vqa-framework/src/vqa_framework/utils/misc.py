...
def add_init_args(
    ...
    """
    Ugly hacky function that takes all the arguments in the initializer <initializer>,
    ...
    for p_name in signature.parameters:
        if p_name not in exclusions + ["self", "kwargs"] and (
            ...
            if param.annotation == bool:
                # Ugly hack to get boolean args to work
                arg_parse_args["type"] = lambda x: bool(strtobool(str(x)))
            # Uglier hack to try and deal with (most) optional arguments
