...
Note: 10 large objects prohibited, b/c I couldn't make it fit.
"""
import argparse
import copy
...
# Opposite for size & material; randomly chosen for shape & color
# These values are used when we need to convert an image with HO, to an
# image not containing HO. For every object exactly matching HO, we replace the
# attributes in HO with the attributes below
...
def generate_required_obj_attributes(ims_with_HO: int, ims_without_HO: int, existing):
    ...
    for found_images in [
        ...
        for ho in HO_choices:
            found_images[ho] = [
                (iid_idx_to_global_idx[glob_idx], loc_idx)
                for (glob_idx, loc_idx) in found_images[ho]
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--val_iid_output_path",
        default="held_out_objects_val_iid.json",
    ...
    parser.add_argument(
        "--test_output_path",
        default="held_out_objects_test.json",
