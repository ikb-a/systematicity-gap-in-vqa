...
if __name__ == "__main__":
    ...
    for split in ["train", "val_iid", "test"]:
        ...
        with open(f"held_out_objects_{split}.json", "r") as infile:
        ...
        for ho in sorted(HO_choices):
            ...
            if split != "test":
                for match_struct, paired_struct in zip(match_ho[ho], match_ho_pair[ho]):
                    ...
                    paired_img = imread(
                        os.path.join(
                            "outputs", "images", split, img_template % paired_img_idx
                        )
                    )
            ...
            for not_match_struct in not_match_ho[ho]:
                ...
                match_img = imread(
                    os.path.join(
                        "outputs", "images", split, img_template % not_match_img_idx
                    )
                )
