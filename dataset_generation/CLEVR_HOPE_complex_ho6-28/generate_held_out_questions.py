...
from held_out_add_utils_generate_questions import (
...
def run_stage1(args, scene_data, train_ho, OUT_DIR, load_stage2: bool = True):
...
def main():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="which HO to generate questions for; 0-indexed, selects an HO from config specified by --ho_tuples_config",
    )
    parser.add_argument(
        "--ho_tuples_config",
    ...
    HO_choices = load_ho_config(args.ho_tuples_config)  # (size, color, shape, material)
    ...
    if args.split == "test":
        ...
        if not args.debug:
            os.makedirs(f"{args.dir}/questions/{split}", exist_ok=True)

        scene_data = load_sg_subset(
            full_sg_path=f"{args.dir}/scenes/CLEVR_held_out_{split}.json",
            objects_file_path=f"{args.dir}/held_out_objects_{split}.json",
            glob_loc_to_img_path=f"{args.dir}/scenes_to_render/glob_loc_index_to_img_index_{split}.json",
            target_ho=test_ho,
            family="match_ho",
        ...
        output_test_path = (
            f"{args.dir}/questions/{split}/CLEVR_held_out_questions_{split}_{str(test_ho)}.json"
            if not args.debug
            else f"{args.dir}_small_test/tmp.json"
        )
