...
"""
NOTE: Blender is a pain -- old version of python & missing libraries.
This file contains the subcomponents of utils that have to be imported
into blender
"""
...
def default_add_empty_filter_options(attribute_map, metadata, num_to_add):
    ...
    while len(attribute_map) < target_size:
        ...
        if k not in attribute_map:
            attribute_map[
                ...
                []
            )  # Note: tuple call maybe not needed, but... odd behaviour & easier to debug


def rejection_heuristic(answer_counts, answer, verbose=False) -> bool:
    """
    Return true if we should reject this answer
    """
...
def find_relate_filter_options(
    ...
    # Right now this is only looking for nontrivial combinations; in some
...
def instantiate_templates_dfs(
    ...
    add_empty_filter_options: Callable[
        [dict, dict, int, dict, dict], None
    ] = lambda fo, me, num, state, nn: default_add_empty_filter_options(fo, me, num),
    ...
    :param max_instances:
    :param verbose:
    :param filter_option_filter: Given a list of possible attribute instantiations in the question, restrict the list to only the "acceptable" choices (e.g., this can be used to prevent any questions using a specific attribute combination)
    :param other_rejection_condition: Given question (dict), metadata (dict),  scene struct (dict), answer (int, bool, or str), and template (dict); decide if we should reject (e.g., if it *doesn't* use a given combination of attributes)
    :param template_name: Unused
    :return:
    """
    ...
    while states:
        ...
        if state["next_template_node"] == len(template["nodes"]):
            ...
            if has_relate:
                ...
                if degen:
                    if verbose:
                        print("Reject because degenerate")
                    continue

            # Any user-specified rejection /requirements:
            if other_rejection_condition(q, metadata, scene_struct, answer, template):
                if verbose:
                    print("Reject because of other rejection condition")
                continue
        ...
        if next_node["type"] in special_nodes:
            if next_node["type"].startswith("relate_filter"):
                ...
                for key in ori_filter_keys:
                    if key not in filtered_ori_keys:
                        filter_options.pop(key)
                ...
                if next_node["type"] == "filter_unique":
                    ...
                    # NOTE: This *might* be a problem for forcing the inclusion of HO when
                    #       HO is not in the scene; may have to be *proactive* with
                    #       the suggestion of HO
                    add_empty_filter_options(
                        filter_options, metadata, num_to_add, state, next_node
                    )

            # NOTE: We can control the question here through filter_option_filter?
            #       [either remove HO, or all but HO depending on case?]
            #       Main caveat: sometimes 4-tuples of attributes,
            #       sometimes (direction, (4-tuple)); hopefully that's it, but can't be sure
            # NOTE: When tweaking program, must remember to PURGE "_output" key from each node (can recreate using qeng)
            filter_option_keys = list(filter_options.keys())
            temp_L = len(filter_option_keys)
            filter_option_keys = filter_option_filter(
                filter_option_keys, template, state
            )

            random.shuffle(filter_option_keys)
            if not next_node["type"].startswith("relate_filter"):
                assert temp_L == len(
                    filter_option_keys
                )  # We filtered earlier, so filtering again shouldn't do anything. Hopefully.
